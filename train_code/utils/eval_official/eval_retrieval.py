...
try:
    ...
    from utils.eval_official import kilt_utils
    from utils.eval_official import eval_downstream
...
def mean_rprecision(guess_item, gold_item, rank_keys):
    ...
    return sum(Rprec_vector) / len(Rprec_vector)
...
def get_ranking_metrics(guess_item, gold_item, ks, rank_keys):
    ...
    mean_Rprec = mean_rprecision(guess_item, gold_item, rank_keys=rank_keys)
...
def compute(gold_dataset, guess_dataset, ks, rank_keys):
    ...
    for guess_item, gold_item in zip(guess_dataset, gold_dataset):
        ...
        result['mean_Rprec'] += ranking_metrics['mean_Rprec']
    ...
    if len(guess_dataset) > 0:
        ...
        result['mean_Rprec'] /= len(guess_dataset)

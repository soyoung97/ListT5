...
class FiDT5(transformers.T5ForConditionalGeneration):
    def __init__(self, config, encoder_output_k):
        super().__init__(config)
        self.encoder_output_k = encoder_output_k
        self.wrap_encoder()
    ...
    def wrap_encoder(self, use_checkpoint=False):
        ...
        """
        self.encoder = EncoderWrapper(self.encoder, encoder_output_k=self.encoder_output_k, use_checkpoint=use_checkpoint)
...
class EncoderWrapper(torch.nn.Module):
    ...
    """
    def __init__(self, encoder, encoder_output_k, use_checkpoint=False):
        super().__init__()
        # soyoung
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        # strip outputs length to encoder_output_k
        #outputs = (outputs[0].view(bsz, self.n_passages*passage_length, -1), ) + outputs[1:]
        # make outputs to [bsize*n_psgs, max_len->k, hidden(768)]
        last_hidden_state = outputs[0][:, :self.encoder_output_k, :].reshape(bsz, self.n_passages*self.encoder_output_k, -1)
